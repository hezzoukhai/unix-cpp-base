   ...
   for (nFldNbr = 0; nFldNbr < 128; nFldNbr ++)
   {
      ...
      { /* Only Mandatory and Conditional Fields */
        ...
        case UAESWITCH_ACQR_ID :
                                AddUaeSwitchField(nFldNbr, sNwInfo, UAESWITCH_INST_ID, strlen(UAESWITCH_INST_ID));
                                break;
        ...
        case ISO_REFERENCE_NBR :
            ...
            case ISO_RESPONSE_CODE :
                                    if (1420 == sMyIsoInfo->nMsgType || 1421 == sMyIsoInfo->nMsgType)
                                          AddUaeSwitchField (nFldNbr, sNwInfo,  "96",  2);
                                    break;
                                 ...
                                 if (GetIsoField (ISO_ICC_DATA, sMyIsoInfo, tmpField, &tmpFieldLen) == SUCCESS)
                                  {
                                     ...
                                     if ( GetIsoIcTag (ISO_TAG_IFD_SERIAL_NUM , &IsoIccInfo, FieldData,  &FieldLen ) == SUCCES )
                                         ...
                                         sprintf  ( sLine ,"===>ISO_TAG_IFD_SERIAL_NUM    = %s ", FieldData );
                                         trace_event ( sLine , PROCESSING );
                                     /** TAG 9F10 **/
                                     memset(FieldData, 0, sizeof(FieldData));
                                     ...
                                     if(GetIsoIcTag (ISO_TAG_APP_CRYPTOGRAM, &IsoIccInfo, FieldData, &FieldLen ) == SUCCES )
                                         AddUaeSwitchIcTag (UAESWITCH_TAG_APP_CRYPTOGRAM, &UaeSwitchIccInfo, FieldData,  FieldLen );
                                           sprintf  ( sLine , "===>ISO_TAG_APP_CRYPTOGRAM   = %s ", FieldData );
                                       trace_event ( sLine , PROCESSING ); 
                                     /** TAG 9F36 **/
                                     memset(FieldData, 0, sizeof(FieldData));
                                     ...
                                     if (GetIsoIcTag (ISO_TAG_TERM_COUNTRY_CODE , &IsoIccInfo, FieldData,  &FieldLen ) == SUCCES )
                                         AddUaeSwitchIcTag (UAESWITCH_TAG_TERM_COUNTRY_CODE , &UaeSwitchIccInfo,FieldData , FieldLen);
                                                 sprintf  ( sLine ,"===>ISO_TAG_COUNTRY_CODE= %s ||LenFD=%d",FieldData,FieldLen);
                                                trace_event ( sLine , PROCESSING );
                                     /** TAG 9A **/
                                     memset(FieldData, 0, sizeof(FieldData));
                                     ...
                                     if (GetIsoIcTag (ISO_TAG_TRANS_AMOUNT , &IsoIccInfo, FieldData,  &FieldLen ) == SUCCES )
                                         ...
                                         sprintf  ( sLine ,"===>SMS_TAG_TRANS_AMOUNT    = %s ", FieldData );
                                          trace_event ( sLine , PROCESSING );
                                     /** TAG 5F2A **/
                                     memset(FieldData, 0, sizeof(FieldData));
                                     ...
                                     if ( GetIsoIcTag (ISO_TAG_DED_FILE_NAME , &IsoIccInfo, FieldData,  &FieldLen ) == SUCCES )
                                     	 ...
                                     	 sprintf  ( sLine ,"---ISO_TAG_DED_FILE_NAME    = %s ", FieldData );
                                     	 trace_event ( sLine , PROCESSING );
                                     /** TAG 9F09 **/
                                     memset(FieldData, 0, sizeof(FieldData));
                                     if ( GetIsoIcTag (ISO_TAG_TERM_APP_VER_NUM , &IsoIccInfo, FieldData,  &FieldLen ) == SUCCES )
                                     	 ...
                                     	 sprintf  ( sLine ,"---ISO_TAG_TERM_APP_VER_NUM    = %s ", FieldData );
                                     	 trace_event ( sLine , PROCESSING );
                                     ...
                                     if ( GetIsoIcTag (ISO_TAG_TRANS_SEQ_NUM , &IsoIccInfo, FieldData,  &FieldLen ) == SUCCES )
                                     	 ...
                                     	 sprintf  ( sLine ,"---ISO_TAG_TRANS_SEQ_NUM    = %s ", FieldData );
                                                 trace_event ( sLine , PROCESSING );
         ...
         case ISO_POS_DATA :
            /*if ( GetUaeSwitchField(nFldNbr, sNwInfo, szWork, &nLen) == SUCCESS
             &&  GetUaeSwitchField(UAESWITCH_MERCHANT_TYPE, sNwInfo, szWork, &nLen) == SUCCESS
               ...
               nLen = ToIsoPosData(szWork, szMerchType, szWork2);
               AddIsoField (nFldNbr, sMyIsoInfo, szWork2, nLen);
            }*/
...
/* Start IBO 12/05/2009  : Get Acquirer Coutry Code from Field 32 */
         case ISO_ACQR_ID :
            if (GetUaeSwitchField(nFldNbr, sNwInfo, szWork, &nLen) == SUCCESS)
	    {
            		AddIsoField (nFldNbr, sMyIsoInfo, szWork, nLen);
            ...
            break;
/* End   IBO 12/05/2009  : Get Acquirer Coutry Code from Field 32 */
         ...
         switch (nFldNbr)
         {
            ...
            case ISO_ADTNL_AMOUNTS :
                                   ...
                                   if ( GetIsoField  (nFldNbr, sMyIsoInfo, szWork, &nLen) == SUCCES ){
                                      ...
                                      AddUaeSwitchField (UAESWITCH_REMAINING_OPEN_TO_USE, sNewNwInfo, szNwAdtnlAmnt, nLen);
                                   }
                                   break;

            case ISO_STLMNT_CODE :
                                  AddUaeSwitchField (nFldNbr, sNewNwInfo, "1", 1); /* in balance */
                                  break;
...
{
   char         szMyRespCode[3+1], szMyProcCode[6+1], szIsoAdtnlAmnt[128];
   char         szWork[256], sLine[MAX_LINE_TRC], szFuncName[] = "BuildRepToNw";
   int          nLen, nFldNbr, nStatus;
   ...
   /* BEGIN TBE - 23/01/2008: Add ISSUER_ACTION_CODE Field */
...
/* ADDED by TBE: 03-07-2007 */
int GetTableIndBis( TSIsoInfo *IsoInfo, char *slProcCode, char *sAutTabInd)
{
       char            sMerchActv         [ 4 + 1 ] ;
       int             nLength                      ;
       char            sLine       [ MAX_LINE_TRC ] ;
       ...
       trace_event(" START     GetTableIndBis()", PROCESSING);
       ...
       sprintf(sLine, " END       GetTableIndBis(%s)", sAutTabInd);
       trace_event(sLine, PROCESSING);
       return (OK);
    ...
    TSUaeSwitchInfo *sNwInfo)
{
  ...
  int		   nLen = 0;
   ...
   sprintf(sLine,"Start GenerateReversal()");
   trace_event(sLine,PROCESSING);
   ...
   memset(sActionCode, 0, sizeof(sActionCode));
    ...
    if (sNwInfo->nMsgType != 210)
    {
   	sprintf(sLine,"No Reversal To Be Generated %d", sNwInfo->nMsgType);
   	trace_event(sLine,TRACE);
   	sprintf(sLine,"End   GenerateReversal(OK)");
   	trace_event(sLine,PROCESSING);
	return(OK);
    }
   if ( GetUaeSwitchField(UAESWITCH_PROC_CODE,sNwInfo,sProcCode,&nLen) != SUCCESS)
   {
   	sprintf(sLine,"No Reversal To Be Generated NO FILED 3 Present" );
   	trace_event(sLine,TRACE);
   	sprintf(sLine,"End   GenerateReversal(OK)");
   	trace_event(sLine,PROCESSING);
	return(OK);
   ...
   if ( memcmp(sProcCode,"01",2) != 0)
   {
   	sprintf(sLine,"No Reversal To Be Generated Processing Code Not 01 %s",sProcCode );
   	trace_event(sLine,TRACE);
   	sprintf(sLine,"End   GenerateReversal(OK)");
   	trace_event(sLine,PROCESSING);
	return(OK);
   ...
   if ( GetUaeSwitchField(UAESWITCH_RESPONSE_CODE,sNwInfo,sActionCode,&nLen) != SUCCESS)
   {
   	sprintf(sLine,"No Reversal To Be Generated NO FILED 39 Present" );
   	trace_event(sLine,TRACE);
   	sprintf(sLine,"End   GenerateReversal(OK)");
   	trace_event(sLine,PROCESSING);
	return(OK);
   ...
   if ( memcmp(sActionCode,"00",2) != 0)
   {
   	sprintf(sLine,"No Reversal To Be Generated Action Code Not 00 %s",sActionCode );
   	trace_event(sLine,TRACE);
   	sprintf(sLine,"End   GenerateReversal(OK)");
   	trace_event(sLine,PROCESSING);
	return(OK);
...
/*
   sNwInfo->nMsgType = 400;
*/
   /***-------- Converting Message UaeSwitch to Iso ---------------------------*/
   if(BuildReqFromNw(nIndexCtx, sAutRevReqMsgBitMap, sNwInfo, &IsoInfo) != OK)
   {
	trace_event("Error Converting Message To Reversal Message",ERROR);
   	sprintf(sLine,"End   GenerateReversal(ERROR)");
   	trace_event(sLine,PROCESSING);
	return(NOK);
	
   }
   IsoInfo.nMsgType = 1200;
   PutIsoField( ISO_MERCHANT_TYPE,&IsoInfo,"6011",4);
   AutomaticReversalProcessing ( nIndexCtx ,
				 /*BEGIN AHM  25/01/2006 */
   				 /*CAPTURE_MODE ,*/
				 ROUTING_MODE,
				 /*END AHM*/
                                 G_sUaeSwitch.F_szRsrcId ,
                                 G_sUaeSwitch.F_szRsrcId ,
                                "ATM" ,
                                &IsoInfo , 
				"4006");
