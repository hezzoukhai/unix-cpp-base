...
---*/
   ...
   trace_event("End   InitCltHsm(OK)", PROCESSING);
   return (nFdConn);
}
...
int InitCltHsmByName(char *sServerName, int nPort)
{
   ...
   ****/
   sprintf(ligne, "Start InitCltHsmByName(%s, %d)", sServerName, nPort);
   trace_event(ligne,  PROCESSING);
   ...
   if ( (nFdTemp = socket(AF_INET, SOCK_STREAM, 0)) == -1 )
   {
      sprintf(ligne, "SOCKET ERROR : errno = %d ", errno);
      trace_event(ligne, ERROR);
      trace_event("End   InitCltHsmByName(NOK)", PROCESSING);
      return(NOK);
   }
   ...
   ****/
   ...
   trace_event("End   InitCltHsmByName(OK)", PROCESSING);
   return (nFdTemp);
}
...
int InitCltCom(char *sServerName, char *sPort)
{
   ...
   ****/
   ...
   trace_event("End   InitCltCom(OK)", PROCESSING);
   return (OK);
}
...
******/
   ...
   {
      sprintf(ligne, "InitSrvComms: SOCKET ERROR : errno = %d ", errno); 
      trace_event(ligne, ERROR);
      return(NOK);
   }
...
int AcceptCall(char  calling_address [ 16])
{
   ...
   sprintf(ligne, "Start AcceptCall()");
   trace_event(ligne, PROCESSING);
   ...
   sprintf(ligne, "End   AcceptCall(%d)", nFdConnect);
   trace_event(ligne, PROCESSING);
   return(OK);
}
...
int InitCltComFd(char *sServerName, char *sPort)
{
   ...
   sprintf(ligne, "Start InitCltCom(%s, %s)", sServerName, sServiceName);
   ****/
   sprintf(ligne, "Start InitCltComFd(%s, %d)", sServerName, nPort);
   trace_event(ligne,  PROCESSING);
   ...
   if ( (nFd = socket(AF_INET, SOCK_STREAM, 0)) == -1 )
   {
      sprintf(ligne, "SOCKET ERROR : errno = %d ", errno);
      trace_event(ligne, ERROR);
      trace_event("End   InitCltComFd(NOK)", PROCESSING);
      return(NOK);
   }
   ...
   ****/
   ...
   trace_event("End   InitCltComFd(OK)", PROCESSING);
   return (nFd);
}
