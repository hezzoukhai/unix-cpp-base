...
unsigned char   ZMK1        [16 + 1];
unsigned char   ZMK2        [16 + 1];
unsigned char   ZMK3        [16 + 1];
unsigned char   ZMK         [16 + 1];
...
memset(sortie2,     0,   sizeof(sortie2));
   ...
   if (argc != 4)
   {
      printf("Usage : <ProgramName> <ZMK1> <ZMK2> <ZMK3>  \n");
      exit(0);
   }
...
memcpy (ZMK1, argv[1], strlen(argv[1]));
memcpy (ZMK2, argv[2], strlen(argv[2]));
memcpy (ZMK3, argv[3], strlen(argv[3]));

fprintf(stderr,"ZMK1             :  %s \n", ZMK1);
fprintf(stderr,"ZMK2             :  %s \n", ZMK2);
fprintf(stderr,"ZMK3             :  %s \n", ZMK3);

    /*  STEP 1  */
x_or(ZMK1, ZMK2, sortie1);

    /*  STEP 2  */
x_or(sortie1, ZMK3, sortie2);

fprintf(stderr,"End   generate_zmk(%.16s) \n", sortie2);
exit (0);
}
