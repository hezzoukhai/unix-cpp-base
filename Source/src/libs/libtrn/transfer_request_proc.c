...
#define SERV_STANDIN_PROC			"TransferRequestProc"
/******************************************************************************/


/******************************************************************************/
int TransferRequestProc(int nIndiceCtx, char*	sTlvBuffer)
{
	...
	trace_event("Start TransferRequestProc()", PROCESSING);
	/******************************************************************************/
	/* 					Initialisation			*/
	/******************************************************************************/
...
*/
	...
	if (memcmp(sExternalResource, NO_ROUTING_RESOURCE, RESOURCE_ID_LEN) == 0)
	{
		...
		trace_event("End TransferRequestProc(OK): Issuer not available", PROCESSING);
		return(nRetCode);
	}
	...
	if ( memcmp(sExternalResource, STAND_IN_RESOURCE, RESOURCE_ID_LEN) != 0 )
	{
		...
		if (CardInSaf(nIndiceCtx, slCardNumber) == OK)
		{
			...
			else
			{
				PutTlvBuffer(AUTO_EVENT_CODE, sTlvBuffer, ISSUER_NOT_AVAILABLE_EVENT, 3);
				nRetCode = AuthLocal(nIndiceCtx, sTlvBuffer);
				trace_event("End TransferRequestProc(OK)", PROCESSING);
				return(nRetCode);
			}
		}
