...
#define SERV_STANDIN_PROC			"ChequeRequestProc"
/******************************************************************************/

/******************************************************************************/
int ChequeRequestProc(int nIndiceCtx, char*	sTlvBuffer)
{
	...
	trace_event("Start ChequeRequestProc()", PROCESSING);
	/******************************************************************************/
	/* 					Initialisation			*/
	/******************************************************************************/
...
*/
	...
	if (memcmp(sExternalResource, NO_ROUTING_RESOURCE, RESOURCE_ID_LEN) == 0)
	{
		...
		trace_event("End ChequeRequestProc(OK): Issuer not available", PROCESSING);
		return(nRetCode);
	}
	...
	if ( memcmp(sExternalResource, STAND_IN_RESOURCE, RESOURCE_ID_LEN) != 0 )
	{
		...
		if (CardInSaf(nIndiceCtx, slCardNumber) == OK)
		{
			...
			else
			{
				PutTlvBuffer(AUTO_EVENT_CODE, sTlvBuffer, ISSUER_NOT_AVAILABLE_EVENT, 3);
				nRetCode = AuthLocalAdm(nIndiceCtx, sTlvBuffer);
				trace_event("End ChequeRequestProc(OK)", PROCESSING);
				return(nRetCode);
			}
		}
	...
	if (memcmp(sExternalResource, STAND_IN_RESOURCE, 6) == 0)
	{
		...
		if (Cheque_Inquiry(nIndiceCtx, sTlvBuffer) != OK)
		{
			return (AuthLocalAdm(nIndiceCtx, sTlvBuffer));
		}
