...
#define SERV_STANDIN_PROC			"PinChangeRequestProc"
/******************************************************************************/


/******************************************************************************/
int PinChangeRequestProc(int nIndiceCtx, char*	sTlvBuffer)
{
	...
	trace_event("Start PinChangeRequestProc()", PROCESSING);
	/******************************************************************************/
	/* 					Initialisation			*/
	/******************************************************************************/
...
*/
	...
	if (memcmp(sExternalResource, NO_ROUTING_RESOURCE, RESOURCE_ID_LEN) == 0)
	{
		...
		trace_event("End PinChangeRequestProc(OK): Issuer not available", PROCESSING);
		return(nRetCode);
	}
	...
	if ( memcmp(sExternalResource, STAND_IN_RESOURCE, RESOURCE_ID_LEN) != 0 )
	{
		...
		if (CardInSaf(nIndiceCtx, slCardNumber) == OK)
		{
			...
			else
			{
				PutTlvBuffer(AUTO_EVENT_CODE, sTlvBuffer, ISSUER_NOT_AVAILABLE_EVENT, 3);
				nRetCode = AuthLocal(nIndiceCtx, sTlvBuffer);
				trace_event("End PinChangeRequestProc(OK)", PROCESSING);
				return(nRetCode);
			}
		}
	...
	if (memcmp(sExternalResource, STAND_IN_RESOURCE, 6) == 0)
	{
		...
		if (PinChange_Inquiry(nIndiceCtx, sTlvBuffer) != OK)
		{
			PutTlvBuffer(AUTO_EVENT_CODE, sTlvBuffer, PIN_NOT_CHANGED, 3);	/*AMER20210809 PLUTONL-3173 Enh21.Q4 MC AN2921*/
			return (AuthLocal(nIndiceCtx, sTlvBuffer));
		}
