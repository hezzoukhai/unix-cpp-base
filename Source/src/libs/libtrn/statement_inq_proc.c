...
/******************************************************************************/
int StatementInquiryProc(int nIndiceCtx, char*	sTlvBuffer)
{
	...
	trace_event("Start StatementInquiryProc()", PROCESSING);
	/******************************************************************************/
	/* 					Initialisation			*/
	/******************************************************************************/
...
/* 	nRetCode = GetOriginalDataElements	(nIndiceCtx,
	...
	*/		
	...
	if (CheckReqTlvFields(nIndiceCtx, sTlvBuffer) != OK)
        {
		trace_event("End StatementInquiryProc(NOK)", PROCESSING);
	...
	{
		trace_event("End StatementInquiryProc(NOK)", PROCESSING);
		return (AuthLocal(nIndiceCtx, sTlvBuffer)); /*IBO20230106 PLUTONL-5598 replace AuthLocalAdm with AuthLocal*/
	}
...
*/
	...
	if (memcmp(sExternalResource, NO_ROUTING_RESOURCE, RESOURCE_ID_LEN) == 0)
	{
		...
		nRetCode = AuthLocal(nIndiceCtx, sTlvBuffer);	/*IBO20230106 PLUTONL-5598 replace AuthLocalAdm with AuthLocal*/
		trace_event("End StatementInquiryProc(NOK): Issuer not available", PROCESSING);
		return(nRetCode);
	}
	...
	if (memcmp(sExternalResource, STAND_IN_RESOURCE, 6) == 0)
	{
/*#if IS_P7_IBE *//*NAB27032023 PLUTONL-6032*/
		if (Statement_Inquiry(nIndiceCtx, sTlvBuffer) != OK)
		{
			trace_event("End StatementInquiryProc(NOK)", PROCESSING);
			return (AuthLocal(nIndiceCtx, sTlvBuffer));	/*IBO20230106 PLUTONL-5598 replace AuthLocalAdm with AuthLocal*/
		}
		...
		nRetCode = AuthLocal(nIndiceCtx, sTlvBuffer);	/*IBO20230106 PLUTONL-5598 replace AuthLocalAdm with AuthLocal*/
/*#else*/
		/*return (ForwardToBackEndIAS ( nIndiceCtx, sTlvBuffer, SERV_ON_RESP_PROC, SERV_ON_TIMEOUT_PROC ));*/
		
	/*	nRetCode = BridgeIASQuery(nIndiceCtx,sTlvBuffer,sTlvBuffer);
	 */
/*#endif*/
	}
	else
	...
	trace_event("End StatementInquiryProc(OK)", PROCESSING);
	return(nRetCode);
}
