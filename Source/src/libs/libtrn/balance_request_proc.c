...
#define SERV_STANDIN_PROC			"BalanceRequestStandinProc" /*NAB31012020*/ /*PLUTONL-1053*/
/******************************************************************************/
/******************************************************************************/
/*NAB31012020*/ /*PLUTONL-1053*/
int BalanceRequestStandinProc(int nIndiceCtx, char*	sTlvBuffer)
{

	/*int				nRetCode;*/

	trace_event("Start BalanceRequestStandinProc()", PROCESSING);


	if (BalanceInquiry(nIndiceCtx, sTlvBuffer) == OK)
		{
			PutTlvBuffer(AUTO_EVENT_CODE, sTlvBuffer, APPROVED_ACTION, 3);
		}
	

	trace_event("End   BalanceRequestStandinProc(OK)", PROCESSING);

	/*return nRetCode;*/ 
	return OK ; /*PLUTONL-1053*/
}
/******************************************************************************/
int BalanceRequestProc(int nIndiceCtx, char*	sTlvBuffer)
{
	...
	trace_event("Start BalanceRequestProc()", PROCESSING);
	/******************************************************************************/
	/* 					Initialisation			*/
	/******************************************************************************/
	...
	if (CheckReqTlvFields(nIndiceCtx, sTlvBuffer) != OK)
		return (AuthLocal(nIndiceCtx, sTlvBuffer));/***SNO280316 replace AuthLocalAdm with AuthLocal */
...
*/
	...
	if (nEventCode != OK)
	{
		...
		return (AuthLocal(nIndiceCtx, sTlvBuffer));/*PLUTONL-3935*/
	}
	...
	if (memcmp(sExternalResource, NO_ROUTING_RESOURCE, RESOURCE_ID_LEN) == 0)
	{
		...
		nRetCode = AuthLocal(nIndiceCtx, sTlvBuffer);/*SNO280316 replace AuthLocalAdm with AuthLocal */
		trace_event("End BalanceRequestProc(OK): Issuer not available", PROCESSING);
		return(nRetCode);
	}
	...
	PutTlvBuffer(AUTO_SERV_STANDIN_ENTRY, sTlvBuffer, SERV_STANDIN_PROC, strlen(SERV_STANDIN_PROC));/*NAB31012020*/ /*PLUTONL-1053*/
	...
	if ( memcmp(sExternalResource, STAND_IN_RESOURCE, RESOURCE_ID_LEN) != 0 )
	{
		...
		if (CardInSaf(nIndiceCtx, slCardNumber) == OK)
		{
			...
			else
			{
				...
				trace_event("End BalanceRequestProc(OK): Card In Saf(Issuer not avail)", PROCESSING);
				return(nRetCode);
			}
		}
...
/*End ICH11102016*/
	...
	trace_event("End BalanceRequestProc(OK)", PROCESSING);
	return(nRetCode);
}
