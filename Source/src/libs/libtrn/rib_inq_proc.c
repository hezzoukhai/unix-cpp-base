...
/******************************************************************************/
int RibInquiryProc(int nIndiceCtx, char*	sTlvBuffer)
{
	...
	trace_event("Start RibInquiryProc()", PROCESSING);
	/******************************************************************************/
	/* 					Initialisation			*/
	/******************************************************************************/
...
/* 	nRetCode = GetOriginalDataElements	(nIndiceCtx,
	...
	if (CheckReqTlvFields(nIndiceCtx, sTlvBuffer) != OK)
        {
		trace_event("End RibInquiryProc(NOK)", PROCESSING);
...
*/
	...
	if (memcmp(sExternalResource, NO_ROUTING_RESOURCE, RESOURCE_ID_LEN) == 0)
	{
		...
		trace_event("End RibInquiryProc(NOK): Issuer not available", PROCESSING);
		return(nRetCode);
	}
	...
	if (memcmp(sExternalResource, STAND_IN_RESOURCE, 6) == 0)
	{
		if (Rib_Inquiry(nIndiceCtx, sTlvBuffer) != OK)
		{
			trace_event("End RibInquiryProc(NOK)", PROCESSING);
			return (AuthLocalAdm(nIndiceCtx, sTlvBuffer));
		}
	...
	trace_event("End RibInquiryProc(OK)", PROCESSING);
	return(nRetCode);
}
