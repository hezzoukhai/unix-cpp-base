...
int AuthorConfirmPrecessing( int nIndiceCtx, TSIsoInfo *IsoInfo , char *sTlvBuffer )
{
	int                         nRetCode;
	...
	/*
	PutTlvBuffer ( AUTO_AUTHO_FLAG         , sTlvBuffer, "C" ,  1);

	if(memcmp(sActionCode, "000", 3) == 0) the BRS saf record should be updated even if the auth is not approved
	{
...
void AuthorConfirmPrecessingEMV(int nIndiceCtx, TSIsoInfo *IsoInfo , char *sTlvBuffer,SStatusInfo *StatusInfo) 
{
	...
	if  ( (!memcmp(sActionCode, APPROVED_ACTION , 3) ) ||   (!memcmp(sActionCode, UPDATE_TRACK3 , 3) ) )
	{
		/*PrintTlvBuffer ( sTlvBuffer );
		nRetCode = UpdateAuthoActivity ( nIndiceCtx , sTlvBuffer );*/
		nRetCode = UpdateAuthoActivityForAtmAdvice ( nIndiceCtx , sTlvBuffer,"1" ); /*IBO20221103 PLUTONL-5364*/
