...
typedef struct STMq_Error_Desc
{
   int     ReasonCode;
   char    ReasonDesc[50];
}TMq_Error_Desc;

TMq_Error_Desc  tab_Mq_Error_Desc[] = 
{
{  0   , "MQRC_NONE                       "  }, 
{  900 , "MQRC_APPL_FIRST                 "  }, 
{  999 , "MQRC_APPL_LAST                  "  }, 
...
int DumpMqReasonDesc ( long nCompCode,long nReasonCode)
{
	...
	nSize = sizeof(tab_Mq_Error_Desc) /sizeof(TMq_Error_Desc);
	nPosition = 0;

	do
	{
		if (tab_Mq_Error_Desc[nPosition].ReasonCode == nReasonCode)
		{
			sprintf(sLine,"Completion Code :[%ld], Reason Code: %ld-%s",nCompCode,nReasonCode,tab_Mq_Error_Desc[nPosition].ReasonDesc);
