...
{   
	...
	{
		DumpMqReasonDesc(CompCode,Reason);
		trace_event("End OpenMQManager(MQCONNX-NOK)", ERROR);
		return(NOK);
	}
...
int OpenIncQueue(MQHCONN* pHcon, MQHOBJ* pHobj, char *szQueue )
{
	...
	{
		DumpMqReasonDesc(CompCode,Reason);
		trace_event("End OpenIncQueue(MQOPEN-NOK)", ERROR);
		return(NOK);
	}
...
int OpenOutgQueue(MQHCONN* pHcon, MQHOBJ* pHobj, char *szQueue )
{
	...
	{
		DumpMqReasonDesc(CompCode,Reason);
		trace_event("END   OpenOutgQueue(MQOPEN-NOK)", ERROR);
		return(NOK);
	}
...
int CloseQueue(MQHCONN Hcon, MQHOBJ* pHobj)
{
	...
	{
		DumpMqReasonDesc(CompCode,Reason);
		trace_event("End CloseQueue(MQCLOSE-NOK)", FATAL);
		return(NOK);
	}
...
int CloseMQManager(MQHCONN* pHcon)
{
	...
	{
		DumpMqReasonDesc(CompCode,Reason);
		trace_event("END   CloseMQManager(MQDISC-NOK)", PROCESSING);
		return(NOK);
	}
...
int ReadMQMsg(MQHCONN Hcon, MQHOBJ Hobj, char *buffer, int nBufLen, int *MsgLen,TSQMsgOpt* pkQMsgOpt, int pnTimer)
{
	...
	while  (CompCode != MQCC_FAILED ) 
	{
		...
		if (CompCode == MQCC_WARNING )
		{
			...
			DumpMqReasonDesc(CompCode,Reason);
			trace_event("End   ReadMQMsg(MQGET-OK) <MQCC_WARNING>", ERROR);
			return (OK);
		}
		...
		{
			DumpMqReasonDesc(CompCode,Reason);
			trace_event("End ReadMQMsg(MQGET-NOK)", ERROR);
			return(ERROR);
		}
...
int WriteMQMsg (MQHCONN Hcon, MQHOBJ Hobj , char *buffer, int nBufLen,TSQMsgOpt* pkQMsgOpt)
{
	...
	{
		DumpMqReasonDesc(CompCode,Reason);
		trace_event("End WriteMQMsg(NOK-MQPUT)" , ERROR);
		return(NOK);
	}
