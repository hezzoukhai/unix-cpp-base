...
/*--------------------------------------------------*/
  ...
  ***********************************************************************/
int To_Ubp_Payment_Msg(int nIndexCtx,
                 						char * NDCBuffer, 
								 						int NDCBuffer_len, 
                 						char      *AcqBank,                 						
                 						TSIsoInfo * P7Buffer	)
{
   ...
   trace_event("Start To_Ubp_Payment_Msg()", PROCESSING);
   ...
   memset(vWbuffer,           0, sizeof(vWbuffer)       );
   memset(amount,           0, sizeof(amount)       );
   memset(lAcqBank,       0, sizeof(lAcqBank)   );/*ICH270715*/
memset(vIsoF22,           0, sizeof(vIsoF22)       );
  ...
  if( strlen(trackii)> 2)
   ...
   if(retour != OK)
   {
      trace_event("End   To_Ubp_Payment_Msg(PROCESSING_ERROR)", PROCESSING);
   ...
   /* (3) PROCESSING_CODE */
   memcpy (lProcessingCode, "500000", 6);
   AddIsoField(ISO_PROC_CODE, P7Buffer, lProcessingCode, 6); 

   /* (4) TRANSACTION_AMOUNT */
   if ( OpCode == OR_GEN_UBP_PAY_SUMMARY )   
		{
			memset( vWbuffer, 0, sizeof(vWbuffer));
			memset( amount, 0, sizeof(amount));
			memset( transCurrency, 0, sizeof(transCurrency));
			
			if ( memcmp(lOCBuffer + 7, "B", 1) == 0 || memcmp(lOCBuffer + 7, "C", 1) == 0 )   /*DEBIT or CREDIT Card amount  */
			{
					Download_Field(NDCBuffer, TR_AMOUNT,	vWbuffer,  NDCBuffer_len);
			    sprintf(sLine, "Buffer B is %s",vWbuffer);
		      trace_event(sLine,TRACE);
		      
		         lTransAmount = atof(vWbuffer)/100; 	
			    
			   if (atoi(vWbuffer) == 0 )
			   {
				  trace_event("End   To_Ubp_Payment_Msg(INVALID_AMOUNT)", PROCESSING);
				  return (INVALID_AMOUNT);
			   }
			  /* lTransAmount = atof(vWbuffer);		RJE */		   
		      
		     }
		    else 
		     {
				   
				   Download_Field(NDCBuffer, 16,       vWbuffer,           NDCBuffer_len);   
				   
					   {
									...
									double    lTransAmount2;
									float     lNoteValue=0;
								
								 	memset(lDom,  0, sizeof(lDom) );
									memset(lDom1, 0, sizeof(lDom1));
								
									len = (int)strlen(vWbuffer);
									lTransAmount2 = 0;
									
								  	while ( i < len-1)
								      	{
										    memset(lDom1, 0, sizeof(lDom1)); /* Note Type */
											memset(lDom, 0, sizeof(lDom));  /* Nbr of Notes */
											
											memcpy(lDom1,vWbuffer+i, 2);	
											memcpy(lDom, vWbuffer+i+2, 3);
															
									   if(GetDenominationInfo(nIndexCtx,term_nbr,lDom1,&lNoteValue) == OK)
										{
											   lTransAmount2 = (lTransAmount2 + (atof(lDom)*lNoteValue) );
										}
																
														i += 5;
									}	
									  lTransAmount = lTransAmount2;
						}
 			}	
						   
						   sprintf(sLine, "lTransAmount...%f",lTransAmount);						   
							trace_event(sLine, PROCESSING);
						   GetTransCurrency(nIndexCtx,term_nbr,lOCBuffer, transCurrency);
						   retour = amount_to_iso(nIndexCtx,&lTransAmount, transCurrency);
						   if(retour == OK)
						      sprintf(amount, "%012.0f", lTransAmount);
						      
					      sprintf(tab_NdcGUbp[nIndex].gTotAmount, "%.02f", lTransAmount/100);

						   
						
		   
		   AddIsoField( ISO_TRANS_AMOUNT, P7Buffer, amount, strlen(amount));

			
		}
   else if ( OpCode == OR_GEN_UBP_PAYMENT )
		{			
			
												sprintf(sLine, ".. tab_NdcGUbp[nIndex].gTotAmount [%s]...",tab_NdcGUbp[nIndex].gTotAmount);
												trace_event(sLine, PROCESSING);
												
										lTransAmount		= atof(tab_NdcGUbp[nIndex].gTotAmount)*100;
										sprintf(amount, "%012.0f", lTransAmount);									
			    
			              	  sprintf(sLine, ".. amount and lTransAmount [%s]...",amount );
												trace_event(sLine, PROCESSING);
   ...
   /* (5) ISO_STLMNT_AMOUNT */
   if ( OpCode == OR_GEN_UBP_PAY_SUMMARY || OpCode == OR_GEN_UBP_PAYMENT )
		 AddIsoField( ISO_TRANS_AMOUNT, P7Buffer, amount, strlen(amount));
	 else	
   ...
   /* (6) BILLING_AMOUNT */
   if ( OpCode == OR_GEN_UBP_PAY_SUMMARY || OpCode == OR_GEN_UBP_PAYMENT )
   	 AddIsoField(ISO_BILLING_AMOUNT, P7Buffer, amount, strlen(amount));
   else
  ...
  sprintf(vIsoF22, "21140121124C");
   ...
   else if(serviceCode[0]=='2' || serviceCode[0]=='6')
      { 
		  vIsoF22[0] = '5';
		  vIsoF22[6] = '8';
	  }
   ...
   if ( 'e' == lEmvBuff[0] )
      ...
      AddIsoField( ISO_POS_DATA, P7Buffer, vIsoF22, 12);
   ...
   /*GetTransCurrency(lOCBuffer, transCurrency); */
   GetTransCurrency(nIndexCtx,term_nbr,lOCBuffer, transCurrency);
   ...
   AddIsoField( ISO_SECURITY_DATA, P7Buffer, tmpField, strlen(tmpField)); 
	...
	sprintf(sLine, "New Payment tab_NdcGUbp[nIndex].gRecord_data   [ %s ]", tab_NdcGUbp[nIndex+RESERVED_THREAD].gRecord_data);
	trace_event(sLine, PROCESSING); 			    

	AddIsoField( ISO_RECORD_DATA, P7Buffer, tab_NdcGUbp[nIndex+RESERVED_THREAD].gRecord_data , strlen(tab_NdcGUbp[nIndex+RESERVED_THREAD].gRecord_data));

				
 

	if(strlen(tab_NdcCtx[nIndex].gSourceAccount) != 0)
		AddIsoField( ISO_ACCOUNT_ID1, P7Buffer, tab_NdcCtx[nIndex].gSourceAccount,strlen(tab_NdcCtx[nIndex].gSourceAccount));
	else
		if ( get_account_number(nIndexCtx,cardNumber, tab_NdcCtx[nIndex].gSourceAccount) == OK )
		AddIsoField( ISO_ACCOUNT_ID1, P7Buffer, tab_NdcCtx[nIndex].gSourceAccount,strlen(tab_NdcCtx[nIndex].gSourceAccount));

	memcpy(tab_NdcGUbp[nIndex].gAccount, 	tab_NdcCtx[nIndex].gSourceAccount, strlen(tab_NdcCtx[nIndex].gSourceAccount));  


	/* (114) CIF  OPCODE*/ 
	sprintf(sLine, "Payment tab_NdcGUbp[nIndex].gOpcode   [ %s ]", tab_NdcGUbp[nIndex].gOpcode);
	trace_event(sLine, PROCESSING); 

	AddIsoField( ISO_RESERVED_ISO_5, P7Buffer, tab_NdcGUbp[nIndex].gOpcode,strlen(tab_NdcGUbp[nIndex].gOpcode));            
   ...
   memset(sLine,           0, sizeof(sLine)       );
	...
	if(OpCode == OR_GEN_UBP_PAY_AMT)  /*Get AMOUNT */
	{
		trace_event("Getting CCDM AMOUNT", PROCESSING); 	
		trace_event("End   To_Ubp_Payment_Msg(NOK)", PROCESSING);
		return (160);
	}

	if(OpCode == OR_GEN_UBP_PAY_SUMMARY) /*SUMMARY SCREEN */
	{
		trace_event("Showing SUMMARY Screen", PROCESSING); 	
		trace_event("End   To_Ubp_Payment_Msg(NOK)", PROCESSING);
		return (170);
	}
  
   trace_event("End   To_Ubp_Payment_Msg(OK)", PROCESSING);
   return (OK);
}
