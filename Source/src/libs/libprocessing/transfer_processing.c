...
/****************************************************************************/
/****************************************************************************/
int TransferReqProcessing	(	int nIndiceCtx,
				                char *sMyResource, 
								char *sAuthoTable, 
								char *sAcquirerBank,
							...
							)
{
    ...
    trace_event("Start TransferReqProcessing()",PROCESSING);
/******************************************************************************/
/* 					Initialisation			*/
/******************************************************************************/
...
/******************************************************************************/
	...
	fprintf(stderr," Impression de TLVBUFFER \n");	
...
/***************************************************************************************/
  /***********************************************************************
  **  nom	: CpTransferReqProcessing						**
  **  desc	: service de traitement de la demande d'autorisation	**
  **		  pour une transaction financiere de transfert de fonds Card Payment	**
  **          Credit Card et Card Pre-paid                                          **
  **  entree: aucune							**
  **  sortie: aucune							**
...
int CpTransferReqProcessing	(	int nIndiceCtx,
									char *sMyResource, 
									char *sAuthoTable, 
									char *sAcquirerBank,
									char *InternationalRoutCheck, 
									...
									TSIsoInfo *IsoInfo  )
{
	...
	char 		cOnOffMode;
    char    	sAlternAmount [ LG_MAX + 1 ];
	char		sShadowLevel	[2 + 1];		
	
    trace_event("Start CpTransferReqProcessing()",PROCESSING);
/******************************************************************************/
/* 					Initialisation			*/
/******************************************************************************/
	...
	memset(sInternalStan        , 0, sizeof(sInternalStan));
	memset(sAlternAmount , 0,  sizeof ( sAlternAmount ));
	memset(sShadowLevel, 0, sizeof(sShadowLevel));
...
/******************************************************************************/
    if ( CheckReqTlvFields ( nIndiceCtx, sTlvBuffer ) != OK )
              return (  AuthorLocalAdm( nIndiceCtx, sTlvBuffer , IsoInfo , 
                                        sMyResource , sPrimResource , 'S' ));
...
/* 	    Check Limits                                                          */
/******************************************************************************/
	/* if( GetTlvBuffer(AUTO_SRCACC_TYPE,sTlvBuffer,sShadowLevel,&nLength)	== SUCCESS) /*RJE 16062014*/
	if( GetTlvBuffer(AUTO_SRCACC_TYPE,sTlvBuffer,sShadowLevel,&nLength)	== SUCCESS)
		if(memcmp(sShadowLevel, "90", 2) != 0)
			if ( CheckLimits(  nIndiceCtx, "OFF", sTlvBuffer, sAlternAmount ) != OK )
              return (  AuthorLocal( nIndiceCtx, sTlvBuffer , IsoInfo , 
                                     sMyResource , sPrimResource , 'N' ));
	...
	if( OnlineTransferInquiry ( nIndiceCtx, sTlvBuffer) != OK)
    {
        trace_event ( "Retour OnlineTransferInquiry NOK ", PROCESSING );
        return (  AuthorLocalAdm( nIndiceCtx, sTlvBuffer , IsoInfo , 
                                sMyResource , sPrimResource , 'N' ));
    }
	...
	if ( CardInSaf ( nIndiceCtx , slCardNumber ) != OK ) {
		/* BEGIN: LHI 25-05-2008 EVENT CODE MUST BE SETTED IN NOK UNTIL ALL CONTROLS ARE OK */
   		...
   		PrintTlvBuffer(sTlvBuffer);
       		...
       		return( AuthorRouting(nIndiceCtx,sTlvBuffer, sPrimResource,nPid,IsoInfo));
        }
        else 
		{
				if (debug_option == 1)
...
/******************************************************************************/
   ...
   trace_event("END CpTransferReqProcessing(OK)",PROCESSING);
   return ( OK );
}
