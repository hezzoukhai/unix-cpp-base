...
#define DAC_POSITION   20          /* Mehdi Elyajizi veification DAC ignor� fonctionnalit� MasterCard et DAC non envoy� syst�matiquement */
#define CVC3_POSITION 24	/* MJA21092011  CONTACTLESS */
#define AVV_POSITION 25		/* MJA20130206  3D-SECURE  */
...
int CheckSecurity ( int nIndiceCtx, char *sTlvBuffer)
{
    ...
    char    		sIccDataTLV         [ LG_MAX + 1 ]; /** MAW20050916 **/
    char    		sIccDataISO         [ LG_MAX + 1 ]; /** MAW20050916 **/
    int     		nIsoICCLen;
    int     		nLength;
    ...
    char   sAtc	          [ 2 + 1 ];
    char	 sAip					[ 4 + 1 ];
/*    char   sDac					[ 4 + 1 ];*/ /* Mehdi Elyajizi veification DAC ignor� fonctionnalit� MasterCard et DAC non envoy� syst�matiquement */
/*    char   sDn					[ 4 + 1 ];*/ /* Mehdi Elyajizi veification IDN ignor� fonctionnalit� MasterCard et IDN non envoy� syst�matiquement */
    char   szWork				[LG_MAX];
    int			result_tvr_cvr;
    /***End Mehdi Elyajizi   15 09 2006 : MYJ231007**/    
...
/************************************************************************/
    ...
    /*    memset ( sDac,			0, sizeof ( sDac)); */ /* Mehdi Elyajizi veification DAC ignor� fonctionnalit� MasterCard et DAC non envoy� syst�matiquement */
    /*		memset ( sDn,			0, sizeof ( sDn));    */ /* Mehdi Elyajizi veification IDN ignor� fonctionnalit� MasterCard et IDN non envoy� syst�matiquement */
    ...
    /* memset ( sPIN_KEY   , 0, sizeof ( sPVKB )); */ /* MKB140208*/
...
/* Read Pos Data                                                           */
/*****************************************************************************/
	GetTlvAutho (AUTO_POS_DATA, &sTlvInfo, sPOS_DATA , &nLength); /* Mehdi Elyajizi 15092006:MYJ231007 */

	/*EBE121126: Move read TrackII bloc after reading equivalent trackII from ICC
 /*Start SWI01122008 Enh Visa iCVV*/
/******************************************************************************************/
	...
	/*if  ( icvv_flag == 'Y' )*/ /*EBE121126*/
	{
		if(GetTlvAutho (AUTO_ICC_DATA, &sTlvInfo, sICC_DATA , &nLength)==SUCCES)
		{
			...
			if (GetTlvPrivate (CHIP_T2_EQUIVALENT_DATA , &tlvInfo, sTrack2Data, &nLen) == SUCCES)
			{
				...
				GetTrack2Data(sTrack2Data,tmpPan,tmpExpiryDate,tmpServiceCode,sPvv,siCvv); /*EBE121126*/
...
/******************************************************************************************/
	...
	if (memcmp(sNetworkCode,"01",2) == 0)   /*checking for Visa */
	{
...
/*---------------------------------------------------------------------------*/
		...
		if ( ( icvv_flag == 'Y' ) && ( (strlen ( siCvv ) != 0) || (sPOS_DATA[6] == '8') ) ) /* 8 : Fallback*/
		{
			...
			else
			{
				if (sPOS_DATA[6] == '8' )
				{
					if ( cvv1_result != OK) /*To avoid double processing of checking fallback trx Jcc want to manage it in the two case cvv_flag=Y and icvv_flag=Y*/
					{
...
/*---------------------------------------------------------------------------*/
	...
	if( GetTlvAutho ( AUTO_PIN_DATA , &sTlvInfo, sPinBlock, &nLength ) == SUCCES)
	{
		if ( ( pin_flag == 'Y' ) && ( nLength != 0 )/* ( strlen ( sPinBlock ) != 0 ) */)
		{
			...
			/* result_pin_info = get_pin_info ( nIndiceCtx, sPan, nPvki,VisaOfset, tOffset, &tPinLength); * Mehdi Elyajizi PCI */
			/* result_pin_info = get_pin_info ( nIndiceCtx, sPan, &nPvki,&VisaOfset, tOffset, &tPinLength); *//*MCK140408 */
			result_pin_info = get_pin_info ( nIndiceCtx, sPan, &nPvki,&VisaOfset, tOffset,sReelPvv, &tPinLength);/*AHA20150609 */
			...
			if ( pin_method ==  PVV_VERIFICATION_METHOD )
			{
				...
				{
					if(memcmp(sReelPvv,"XXXX",4)==0)
					{
						sprintf(sPvki,"%01d",nPvki);            
						pvki = sPvki[0];
						memcpy(sPvv1,sPvv + 1,4);
						Pvv = atoi(sPvv1) + VisaOfset;
				...
				{
					pvki = sPvv[0];
					memcpy(sPvv1,sPvv + 1,4); /**MKB081107**/
				}
...
/** END   MAW20050916 **/
	...
	FillSecurityResult ( pin_result  , sSecurityResults , PIN_POSITION );
	FillSecurityResult ( crpt_result  , sSecurityResults , CRPT_POSITION );
	FillSecurityResult ( icvv_result  , sSecurityResults , ICVV_POSITION ); /* Mehdi Elyajizi 26092006: MYJ231007 **/  
/* BEGIN: LHI 25-05-2008 EVENT CODE MUST BE SETTED IN NOK UNTIL ALL CONTROLS ARE OK
  PutTlvAutho (AUTO_EVENT_CODE, &sTlvInfo , APPROVED_ACTION , 3);
	*/

	PutTlvAutho(AUTO_SECURITY_VER_RESULTS,&sTlvInfo,sSecurityResults, 32 ); /***Start Mehdi Elyajizi 15092006 32 au lieu de 20*******/
	AuthoBuildTlv(sTlvBuffer, &sTlvInfo);
