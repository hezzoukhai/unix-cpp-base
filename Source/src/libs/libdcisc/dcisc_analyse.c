...
int   AnalyseDcisc(char * buffer_rec, TSDciscInfo *msgInfo)
{
	...
	memcpy( msgInfo->sData, sPtr, MAX_DCISC_DATA_LEN);
	...
	for (i = 0; i < iMapCtr * 64; ++i)
	{
		...
		{
			switch ( GetDciscFieldType(i) )
			{
			case DCISC_FIX_BIN   :
			case DCISC_FIX_ALPHA :
				nBytes = GetDciscFieldLength(i);
				msgInfo->nFieldPos[i+1] = msgInfo->nFieldPos[i] + nBytes;
				break;

			case DCISC_FIX_HEX :                           
				nDigits = GetDciscFieldLength(i);
				nBytes = ( nDigits + 1)/2;
				msgInfo->nFieldPos[i+1] = msgInfo->nFieldPos[i] + nBytes;
				break;
	...
	for (i = iMapCtr*64; i< MAX_DCISC_FIELDS; i++)
		msgInfo->nFieldPos[i+1] = msgInfo->nFieldPos[iMapCtr*64];

	return OK;
}

int AnalyseDciscIc(char * buffer, int nLength, TSTagDcisc * tagInfo)
{
	char  *sPtr;
	int   offset;
	...
	while (offset < nLength)
	{
		...
		nTagIndice = GetDciscTagIndice( szTagName);
		if ( nTagIndice >= 0)
		{
