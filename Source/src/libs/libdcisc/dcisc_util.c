...
#include <iso_dcisc.h>
#include <dcisc_param.h>
#include <tools.h>
#include <define.h>
#include <string.h>

#include <dcisc_ictag.h>


int GetDciscField(int field_no, TSDciscInfo * msgInfo, char *data, int *nLength)
{
	int length;
	int nBytes;
	char szIsoFldLen[4];	
	...
	nMaxLength = GetDciscFieldLength(field_no);
	switch (GetDciscFieldType(field_no))
	{
	case DCISC_LL_BIN :
		...
		{
			sprintf(sLine,"GetDciscField(%d) invalid length Max: %d VS found %d",field_no,nMaxLength,length);
...
int AddDciscField(int field_no, TSDciscInfo *msgInfo, char *data, int nLength)
{
	int nBytes  = 0;
	int i;
	char szIsoFldLen[4];	
	...
	case DCISC_FIX_ALPHA :
		...
		{
			sprintf(sLine,"AddDciscField(%d) invalid length Max: %d VS %d",field_no,nMaxLength,nLength);
			trace_event(sLine,ERROR);
			return(ECHEC);
		}
	...
	for (i=field_no; i< MAX_DCISC_FIELDS; i++)
...
int InsertDciscField(int field_no, TSDciscInfo *msgInfo, char *data, int nLength)
{
	int  nBytes   = 0;
	int  nBytes1  = 0;
	int  i;
	...
	case DCISC_FIX_ALPHA :
		if ( nMaxLength != nLength)
		{
			sprintf(sLine,"InsertDciscField(%d) invalid length Max: %d VS %d",field_no,nMaxLength,nLength);
			trace_event(sLine,ERROR);
			return(ECHEC);
		}
		nBytes = nLength;
	...
	for (i = MAX_DCISC_FIELDS-1; i > field_no; i--)
	{
...
int PutDciscField(int field_no, TSDciscInfo *msgInfo, char *data, int nLength)
{
	int  nBytes   = 0;
	int  nBytes1  = 0;
	...
	if ( CheckBit( msgInfo->sBitMap, field_no))
	{
		...
		case DCISC_FIX_ALPHA :
			if ( nMaxLength != nLength)
			{
				sprintf(sLine,"PutDciscField(%d) invalid length Max: %d VS %d",field_no,nMaxLength,nLength);
				trace_event(sLine,ERROR);
				return(ECHEC);
			}
			nBytes = nLength;
		...
		return ( InsertDciscField (field_no, msgInfo, data, nLength));
...
/****************************************************************************/
int GetDciscIcTag (char  *tag_name, TSTagDcisc *tagInfo, char *data, int *length)
{
	int  nIndice ;
	int  nLenTag ;
	int  nDigits;
	...
	if ( (nIndice = GetDciscTagIndice(tag_name)) == -1)
		return(ECHEC);
	...
	*length = (*(tagInfo->sChipData + tagInfo->nPosTag[nIndice]+nLenTag));
	switch (GetDciscTagType(nIndice) )
	{
	case DCISC_TAG_FIX_BIN :
	case DCISC_TAG_FIX_ALPHA :
		memcpy( data,tagInfo->sChipData + tagInfo->nPosTag[nIndice] +nLenTag + 1,
			*length);
		break;

	case DCISC_TAG_VAR_HEX :
	...
	case DCISC_TAG_VAR_BIN :
	case DCISC_TAG_VAR_ALPHA :
		memcpy( data, tagInfo->sChipData +tagInfo->nPosTag[nIndice] +nLenTag + 1,
			*length);
		break;
...
int AddDciscIcTag (char  *tag_name, TSTagDcisc *tagInfo, char *data, int length)
{
	int nIndice;
	int nLenTag;
	int nLength;
	...
	if(!memcmp(tag_name, DCISC_TAG_TVR, 4))
		nLenTag = 1;
	...
	case DCISC_TAG_FIX_BIN :
	case DCISC_TAG_FIX_ALPHA :
		if ( GetDciscTagLength(nIndice) != length)
			return(ECHEC);
...
int PutDciscIcTag (char  *tag_name, TSTagDcisc *tagInfo, char *data, int length)
{
	int nIndice;
	int nLenTag;
	int nLength;
	...
	char szLastPart  [ MAX_DCISC_CHIP_LEN ];
	int nOldTagLen, i;
	...
	*(tagInfo->sChipData + nPutPos + nLenTag)    = nDigits;
	...
	else
	{
		...
		for (i = nIndice + 1; i < MAX_DCISC_CHIP_TAG; i++)
		{
...
int DciscBuildIcFld (char * buffer_snd, TSTagDcisc *tagInfo)
{
