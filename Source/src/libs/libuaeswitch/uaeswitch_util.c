/*
-----------------------------------------------------------------------------------------------------------------------------------------
-- Description : 
	-------------------------------------------------------------------------------------------------------------------------------------
	
-----------------------------------------------------------------------------------------------------------------------------------------
-- Modification History : 
	-------------------------------------------------------------------------------------------------------------------------------------
	-- Version    Date          	Author       		Jalon                     Activity                          Comment
	-------------------------------------------------------------------------------------------------------------------------------------
	-- V3.0.0     29/12/2014    	HPS                                                                           	Initial version
-----------------------------------------------------------------------------------------------------------------------------------------
*/
#include <stdio.h>
#include <stdlib.h>
#include <iso_uaeswitch.h>
#include <iso_uaeswitch_fields.h>
#include <tools.h>
#include <define.h>


typedef struct SICCTagData
{
    char  *sTagName;
    char  *sTagLabel;
} TSICCTagData;

TSICCTagData iccTagData[] ={
 {"50  "  , "APPLICATION_LABEL        ..."},
 {"57  "  , "TRACK_2_DATA             ..."},
 {"5A  "  , "APPL_PRIM_ACCOUNT        ..."},
 {"71  "  , "ISS_SCRIPT_TEMPLATE_1    ..."},
 {"72  "  , "ISS_SCRIPT_TEMPLATE_2    ..."},
 {"82  "  , "APP_INTERCH_PROFILE      ..."},
 {"84  "  , "DEDICATES_FILE_NAME      ..."},
 {"86  "  , "ISSUER_SCRIPT_COMMAND    ..."},
 {"8A  "  , "AUTH_RESPONSE_CODE       ..."},
 {"8C  "  , "CARD_RISK_MANAG_CDOL1    ..."},
 {"91  "  , "ISSUER_AUTHENT_DATA      ..."},
 {"95  "  , "TERMINAL_VERIF_RESULTS   ..."},
 {"9A  "  , "TRANSACTION_DATE         ..."},
 {"9C  "  , "TRANSACTION_TYPE         ..."},
 {"5F34"  , "APP_PAN_SEQ_NUMBER       ..."},
 {"5F2A"  , "TRANSACTION_CUR_CODE     ..."},
 {"9F02"  , "TRANSACTION_AMOUNT       ..."},
 {"9F03"  , "OTHER_AMOUNT             ..."},
 {"9F06"  , "APP_SELECTED             ..."},
 {"9F09"  , "TERM_APPL_VERSION_NBR    ..."},
 {"9F10"  , "ISSUER_APPL_DATA         ..."},
 {"9F1A"  , "TERMINAL_COUNTRY_CODE    ..."},
 {"9F1E"  , "INTER_DEVICE_SERIAL_NBR  ..."},
 {"9F26"  , "APPLICATION_CRYPTOGRAM   ..."},
 {"9F27"  , "CRYPTOGRAM_INF_DATA      ..."},
 {"9F33"  , "TERMINAL_CAPABILITIES    ..."},
 {"9F34"  , "CVM_RESULTS              ..."}, 
 {"9F35"  , "TERMINAL_TYPE            ..."},
 {"9F36"  , "APP_TRANS_COUNTER        ..."},
 {"9F37"  , "UNPREDICTABLE_NUMBER     ..."},
 {"9F41"  , "TRANS_SEQ_NUMBER         ..."},
 {"9F53"  , "TRANS_CATEGORY_CODE      ..."},
};

int GetUaeSwitchField(int field_no, TSUaeSwitchInfo *msgInfo, char *data, int *nLength)
{
   int length;
   int nBytes;

   if (CheckBit (msgInfo->sBitMap, field_no))
   {
      switch (GetUaeSwitchFieldType(field_no))
      {
      case UAESWITCH_VAR_BIN :
      case UAESWITCH_VAR_BITMAP :
         length = *(msgInfo->sData + msgInfo->nFieldPos[field_no]);
         memcpy(data, msgInfo->sData + msgInfo->nFieldPos[field_no]+1,length);
         *nLength = length;
         break;

      case UAESWITCH_FIX_BIN :
         length = GetUaeSwitchFieldLength(field_no);
         memcpy( data, msgInfo->sData + msgInfo->nFieldPos[field_no], length);
         *nLength = length;
         break;

      case UAESWITCH_FIX_HEX :                           
         length = GetUaeSwitchFieldLength(field_no);
         nBytes = HexToAscii( msgInfo->sData+msgInfo->nFieldPos[field_no], 
                                                             data, length);
         data[length] = '\0';
         *nLength = length;
         break;

      case UAESWITCH_VAR_HEX :                           
         length = *(msgInfo->sData + msgInfo->nFieldPos[field_no]);
         nBytes = HexToAscii(msgInfo->sData+msgInfo->nFieldPos[field_no]+1, 
                                                            data, length); 
         data[length] = '\0';
         *nLength = length;
         break;

      case UAESWITCH_FIX_BCD :                           
         length = GetUaeSwitchFieldLength(field_no);
         nBytes = BcdToAscii(msgInfo->sData+msgInfo->nFieldPos[field_no], 
                                                            data, length); 
         data[length] = '\0';
         *nLength = length;
         break;

      case UAESWITCH_VAR_BCD :                           
         length = *(msgInfo->sData + msgInfo->nFieldPos[field_no]);
         nBytes = BcdToAscii(msgInfo->sData+msgInfo->nFieldPos[field_no]+1, 
                                                            data, length); 
         data[length] = '\0';
         *nLength = length;
         break;

      case UAESWITCH_VAR_SBCD :                           
         length = *(msgInfo->sData + msgInfo->nFieldPos[field_no]);
         nBytes = BcdToAscii(msgInfo->sData+msgInfo->nFieldPos[field_no]+1, 
                                                            data, 2*length); 
         data[2*length] = '\0';
         *nLength = 2*length;
         break;

      case UAESWITCH_FIX_ALPHA :                           
         length = GetUaeSwitchFieldLength(field_no);
         EbcdicToAscii(msgInfo->sData+msgInfo->nFieldPos[field_no], 
                                                            data, length); 
         data[length] = '\0';
         *nLength = length;
         break;

      case UAESWITCH_VAR_ALPHA :                           
         length = *(msgInfo->sData + msgInfo->nFieldPos[field_no]);
         EbcdicToAscii(msgInfo->sData+msgInfo->nFieldPos[field_no]+1, 
                                                            data, length); 
         data[length] = '\0';
         *nLength = length;
         break;

      default :
         return(ECHEC);
         break;
         
      }
      return(SUCCES);
   }
   return(ECHEC);
}

int AddUaeSwitchField(int field_no, TSUaeSwitchInfo *msgInfo, char *data, int nLength)
{
   int i,nBytes  = 0;
   char  sLine[MAX_LINE_TRC];

   if ( CheckBit( msgInfo->sBitMap, field_no))
      return(ECHEC);

   msgInfo->nFieldPos[field_no] = msgInfo->nLength;
   switch (GetUaeSwitchFieldType(field_no))
   {
   case UAESWITCH_FIX_BIN :
      nBytes = GetUaeSwitchFieldLength(field_no);
      memcpy( msgInfo->sData+msgInfo->nLength, data, nBytes);
      break;

   case UAESWITCH_VAR_BIN :
   case UAESWITCH_VAR_BITMAP :
      *(msgInfo->sData + msgInfo->nLength) = nLength;
      msgInfo->nLength++;
      memcpy( msgInfo->sData+msgInfo->nLength, data, nLength);
      nBytes = nLength;
      break;

   case UAESWITCH_FIX_BCD :
      if ( GetUaeSwitchFieldLength(field_no) != nLength )
         return(ECHEC);
      nBytes = AsciiToBcd( data, msgInfo->sData+msgInfo->nLength, nLength);
      break;

   case UAESWITCH_FIX_HEX :
      if ( GetUaeSwitchFieldLength(field_no) != nLength)
         return(ECHEC);
      nBytes = AsciiToHex( data, msgInfo->sData+msgInfo->nLength, nLength);
      break;

   case UAESWITCH_FIX_ALPHA :
      if ( GetUaeSwitchFieldLength(field_no) != nLength)
         return(ECHEC);
      AsciiToEbcdic( data, msgInfo->sData+msgInfo->nLength, nLength);
      nBytes = nLength;
      break;

   case UAESWITCH_VAR_HEX :
      *(msgInfo->sData + msgInfo->nLength) = nLength;
      msgInfo->nLength++;
      nBytes = AsciiToHex( data, msgInfo->sData+msgInfo->nLength, nLength);
      break;

   case UAESWITCH_VAR_BCD :
      *(msgInfo->sData + msgInfo->nLength) = nLength;
      msgInfo->nLength++;
      nBytes = AsciiToBcd( data, msgInfo->sData+msgInfo->nLength, nLength);
      break;

   case UAESWITCH_VAR_SBCD :
      *(msgInfo->sData + msgInfo->nLength) = (nLength+1)/2;
      msgInfo->nLength++;
      nBytes = AsciiToBcd( data, msgInfo->sData+msgInfo->nLength, nLength);
      break;

   case UAESWITCH_VAR_ALPHA :
      *(msgInfo->sData + msgInfo->nLength) = nLength;
      msgInfo->nLength++;
      AsciiToEbcdic(data, msgInfo->sData+msgInfo->nLength, nLength);
      nBytes = nLength;
      break;

   default :
      return(ECHEC);
   }
   msgInfo->nLength += nBytes;
   for (i=field_no; i< MAX_UAESWITCH_FIELDS; i++)
       msgInfo->nFieldPos[i+1] = msgInfo->nLength;
   SetBit(msgInfo->sBitMap, field_no+1);
   if ( (field_no >64) /* && (field_no < 128) */)
      SetBit(msgInfo->sBitMap, 1);
   if (field_no >= 128)
      SetBit(msgInfo->sBitMap, 65);
   return(SUCCES);
}

int InsertUaeSwitchField(int field_no, TSUaeSwitchInfo *msgInfo, char *data, int nLength)
{
   int nBytes  = 0;
   int i;
   char szUaeSwitchData [999];

   if ( CheckBit( msgInfo->sBitMap, field_no))
      return(ECHEC);

   switch (GetUaeSwitchFieldType(field_no))
   {
   case UAESWITCH_FIX_BIN :
      nBytes = GetUaeSwitchFieldLength(field_no);
      memcpy( szUaeSwitchData, data, nBytes);
      break;

   case UAESWITCH_VAR_BIN :
   case UAESWITCH_VAR_BITMAP :
      *(szUaeSwitchData) = nLength;
      nBytes = 1;
      memcpy( szUaeSwitchData+1, data, nLength);
      nBytes += nLength;
      break;

   case UAESWITCH_FIX_BCD :
      if ( GetUaeSwitchFieldLength(field_no) != nLength )
         return(ECHEC);
      nBytes = AsciiToBcd( data, szUaeSwitchData, nLength);
      break;

   case UAESWITCH_FIX_HEX :
      if ( GetUaeSwitchFieldLength(field_no) != nLength)
         return(ECHEC);
      nBytes = AsciiToHex( data, szUaeSwitchData, nLength);
      break;

   case UAESWITCH_FIX_ALPHA :
      if ( GetUaeSwitchFieldLength(field_no) != nLength)
         return(ECHEC);
      AsciiToEbcdic( data, szUaeSwitchData, nLength);
      nBytes = nLength;
      break;

   case UAESWITCH_VAR_SBCD :
      *(szUaeSwitchData) = (nLength+1)/2;
      nBytes = AsciiToBcd( data, szUaeSwitchData+1, nLength);
      nBytes++;
      break;

   case UAESWITCH_VAR_BCD :
      *(szUaeSwitchData) = nLength;
      nBytes = AsciiToBcd( data, szUaeSwitchData+1, nLength);
      nBytes++;
      break;

   case UAESWITCH_VAR_HEX :
      *(szUaeSwitchData) = nLength;
      nBytes = AsciiToHex( data, szUaeSwitchData+1, nLength);
      nBytes++;
      break;

   case UAESWITCH_VAR_ALPHA :
      *(szUaeSwitchData) = nLength;
      nBytes =1;
      AsciiToEbcdic(data, szUaeSwitchData+1, nLength);
      nBytes += nLength;
      break;

   default :
      return(ECHEC);
   }
   for (i = MAX_UAESWITCH_FIELDS-1; i > field_no; i--)
   {
      memcpy (msgInfo->sData+msgInfo->nFieldPos[i]+nBytes,
              msgInfo->sData+msgInfo->nFieldPos[i],
              msgInfo->nFieldPos[i+1] - msgInfo->nFieldPos[i] );
      msgInfo->nFieldPos[i+1] += nBytes;
   }
   memcpy(msgInfo->sData+msgInfo->nFieldPos[field_no], szUaeSwitchData, nBytes);
   msgInfo->nFieldPos[field_no+1] += nBytes;
   msgInfo->nLength += nBytes;
   SetBit(msgInfo->sBitMap, field_no+1);
   if ( (field_no >64) )
      SetBit(msgInfo->sBitMap, 1);
   if (field_no >= 128)
      SetBit(msgInfo->sBitMap, 65);
   return(SUCCES);
}

int PutUaeSwitchField(int field_no, TSUaeSwitchInfo *msgInfo, char *data, int nLength)
{

   int  nBytes   = 0;
   int  nBytes1  = 0;
   int  i, nOldFldLen,nOldLen;
   char szUaeSwitchData [999];	
   char szWork    [LG_MAX];	
   char szIsoFldLen[4];	

   if ( CheckBit( msgInfo->sBitMap, field_no))
   {
      switch (GetUaeSwitchFieldType(field_no))
      {
      case UAESWITCH_FIX_BIN :
         nBytes = GetUaeSwitchFieldLength(field_no);
         memcpy( szUaeSwitchData, data, nBytes);
         break;

      case UAESWITCH_VAR_BIN :
      case UAESWITCH_VAR_BITMAP :
         *(szUaeSwitchData) = nLength;
         nBytes = 1;
         memcpy( szUaeSwitchData+1, data, nLength);
         nBytes += nLength;
         break;

      case UAESWITCH_FIX_BCD :
         if ( GetUaeSwitchFieldLength(field_no) != nLength )
            return(ECHEC);
         nBytes = AsciiToBcd( data, szUaeSwitchData, nLength);
         break;

      case UAESWITCH_FIX_HEX :
         if ( GetUaeSwitchFieldLength(field_no) != nLength)
            return(ECHEC);
         nBytes = AsciiToHex( data, szUaeSwitchData, nLength);
         break;

      case UAESWITCH_FIX_ALPHA :
         if ( GetUaeSwitchFieldLength(field_no) != nLength)
            return(ECHEC);
         AsciiToEbcdic( data, szUaeSwitchData, nLength);
         nBytes = nLength;
         break;

      case UAESWITCH_VAR_SBCD :
         *(szUaeSwitchData) = (nLength+1)/2;
         nBytes = AsciiToBcd( data, szUaeSwitchData+1, nLength);
         nBytes++;
         break;

      case UAESWITCH_VAR_BCD :
         *(szUaeSwitchData) = nLength;
         nBytes = AsciiToBcd( data, szUaeSwitchData+1, nLength);
         nBytes++;
         break;

      case UAESWITCH_VAR_HEX :
         *(szUaeSwitchData) = nLength;
         nBytes = AsciiToHex( data, szUaeSwitchData+1, nLength);
         nBytes++;
         break;

      case UAESWITCH_VAR_ALPHA :
         *(szUaeSwitchData) = nLength;
         nBytes =1;
         AsciiToEbcdic(data, szUaeSwitchData+1, nLength);
         nBytes += nLength;
         break;

      default :
         return(ECHEC);
      }

      nOldFldLen = msgInfo->nFieldPos[field_no +1] - 
                   msgInfo->nFieldPos[field_no];
      memset(szWork, 0, sizeof(szWork));
      nOldLen =  msgInfo->nLength - msgInfo->nFieldPos[field_no+1];
      memcpy(szWork, msgInfo->sData + msgInfo->nFieldPos[field_no+1], nOldLen);

      for (i = MAX_UAESWITCH_FIELDS-1; i > field_no; i--)
      {
         msgInfo->nFieldPos[i+1] += nBytes-nOldFldLen;
      }
      memcpy(msgInfo->sData+msgInfo->nFieldPos[field_no], szUaeSwitchData, nBytes);
      msgInfo->nFieldPos[field_no+1] += nBytes-nOldFldLen;
      msgInfo->nLength += nBytes-nOldFldLen;
      SetBit(msgInfo->sBitMap, field_no+1);
      memcpy(msgInfo->sData + msgInfo->nFieldPos[field_no + 1], szWork, nOldLen);
      if ( (field_no >64) && (field_no < 128))
         SetBit(msgInfo->sBitMap, 1);
      return(SUCCES);
   }
   else
      return ( InsertUaeSwitchField (field_no, msgInfo, data, nLength));
}

int UaeSwitchBuildMsg (char * buffer_snd, TSUaeSwitchInfo *msgInfo)
{
   char szMsgType[5];
   int  nBytes;
   int  offset;
   int  iMapCtr;

   offset = 0;
   AsciiToEbcdic( "ISO8583-1987001000000", buffer_snd + offset, UAESWITCH_HEADER_LEN);
   offset += UAESWITCH_HEADER_LEN ;

   memset(szMsgType, 0, 5);
   sprintf(szMsgType, "%.4d", msgInfo->nMsgType);
   nBytes = AsciiToBcd( szMsgType, buffer_snd + offset, 4);
   offset += nBytes ;

   iMapCtr = 1;
   while ( ( msgInfo->sBitMap[(iMapCtr - 1) * 8] & 0x80) )
      ++iMapCtr;
   memcpy( buffer_snd+offset, msgInfo->sBitMap, iMapCtr*8);
   memcpy( buffer_snd+offset + (iMapCtr*8), msgInfo->sData, msgInfo->nLength);
   return( offset + (iMapCtr*8) + msgInfo->nLength);
}


int AddUaeSwitchIcTag (char  *tag_name, TSTagUaeSwitch *tagInfo, char *data, int length)
{
   int nIndice;
   int nLenTag;
   int nLength;
   int nDigits;
   
   if ( (nIndice = GetUaeSwitchTagIndice(tag_name)) == -1)
      return(ECHEC);

  if (tagInfo->nPresent[nIndice] == PRESENT)
      return(ECHEC);


   AsciiToHex(tag_name, tagInfo->sChipData + tagInfo->nLength, 4);


   if (  (unsigned char) (*(tagInfo->sChipData + tagInfo->nLength) & 0x1F ) ==  (unsigned char) 0x1F)
      nLenTag = 2;
   else
      nLenTag = 1;

   switch (GetUaeSwitchTagType(nIndice) )
   {
   case UAESWITCH_TAG_FIX_BIN :
   case UAESWITCH_TAG_FIX_ALPHA :
      if ( GetUaeSwitchTagLength(nIndice) != length)
         return(ECHEC);
      memcpy( tagInfo->sChipData + tagInfo->nLength +nLenTag + 1, data, length);
      nDigits = length;
      break;

   case UAESWITCH_TAG_VAR_HEX :
      nDigits = AsciiToHex( data,
                            tagInfo->sChipData + tagInfo->nLength + nLenTag+1,
                            length);
      break;
   case UAESWITCH_TAG_VAR_BCD :
      nDigits = AsciiToBcd( data,
                            tagInfo->sChipData + tagInfo->nLength + nLenTag+1,
                            length);
      break;

   case UAESWITCH_TAG_FIX_BCD :
      if ( GetUaeSwitchTagLength(nIndice) != length)
         return(ECHEC);
      nDigits = AsciiToBcd( data,
                            tagInfo->sChipData + tagInfo->nLength + nLenTag+1,
                            length);
      break;

   case UAESWITCH_TAG_VAR_BIN :
   case UAESWITCH_TAG_VAR_ALPHA :
      memcpy( tagInfo->sChipData + tagInfo->nLength +nLenTag + 1, data, length);
      nDigits = length;
      break;

   }

  	*(tagInfo->sChipData + tagInfo->nLength + nLenTag)    = nDigits;
   	tagInfo->nPosTag [ nIndice ] = tagInfo->nLength;

  	tagInfo->nPresent[ nIndice ] = PRESENT;
   	tagInfo->nLength +=nDigits+nLenTag + 1;
   	return(SUCCES);
	
}

int UaeSwitchBuildIcFld (char * buffer_snd, TSTagUaeSwitch *tagInfo)
{
   memcpy (buffer_snd, tagInfo->sChipData, tagInfo->nLength);
   return(tagInfo->nLength);
}

int GetIccTagLabl (char * sTagName, char * sTagLabl)
{
 int i=0;
 
 for (i=0; i<(sizeof(iccTagData)/sizeof(iccTagData[0])); i++)
 {
   if (memcmp(iccTagData[i].sTagName, sTagName, strlen(sTagName)) ==0)
   {
    memcpy(sTagLabl,iccTagData[i].sTagLabel,strlen(iccTagData[i].sTagLabel));
    break;
   }
 }
 
 if (strlen(sTagLabl) ==0 )
 {
    memcpy(sTagLabl,sTagName,strlen(sTagName));
 }
 
 return(OK);
}