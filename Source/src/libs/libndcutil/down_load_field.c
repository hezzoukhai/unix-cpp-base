...
/*********************************************************************/
/*********************************************************************/
int CompactPIN(unsigned char   *pin,unsigned char   *pinCompacted)
{
	char            sLine[MAX_LINE_TRC];
	int             cpt;
	...
	if (debug_option == 1)
		...
		sprintf(sLine, "Start CompactPIN()");
	...
	int             Lg)
{
	...
	sprintf(sLine, "Debut Download_Field(%d,%d)", Field_Number, Lg);
	trace_event(sLine, PROCESSING);
	...
	while ((j < Lg) && (Buffer[j] != FS)) {
		if (
			(TR_EMV_DATA == Field_Number && 0x00 == Buffer[j])
			||
			(TR_EMV_DATA == Field_Number && GS 	 == Buffer[j])
			||
			(TR_CASH_DEP_WN == Field_Number && GS 	 == Buffer[j])	/* TBO19052011 Added */			
	...
	case TR_TRACKII:
		if (debug_option == 1)
			...
			sprintf(sLine, "Fin   Download_Field(TRACKII)");
		trace_event(sLine, PROCESSING);
		break;
	case TR_TRACKIII:
		if (debug_option == 1)
			...
			sprintf(sLine, "Fin   Download_Field(TRACKIII)");
		trace_event(sLine, PROCESSING);
		break;
	case TR_PIN:
		if (debug_option == 1)
			...
			sprintf(sLine, "Fin   Download_Field(TR_PIN)");
		trace_event(sLine, PROCESSING);
		break;
	/*AMER 20160728 Start: PADSS Fix*/
	case TR_FIRST_PIN:
                if (debug_option == 1)
                        sprintf(sLine, "Fin   Download_Field(%.30s)", Field);
                else
                        sprintf(sLine, "Fin   Download_Field(TR_FIRST_PIN)");
                trace_event(sLine, PROCESSING);
                break;
	case TR_SECOND_PIN:
                if (debug_option == 1)
                        ...
                        sprintf(sLine, "Fin   Download_Field(TR_SECOND_PIN)");
                trace_event(sLine, PROCESSING);
                break;

	/*AMER 20160728 End: PADSS Fix*/
	default:
		sprintf(sLine, "Fin   Download_Field(%.30s)", Field);
		trace_event(sLine, PROCESSING);
		return (OK);
...
/*
void 
	Load_Pin_Block(pin_rcv, calcul_pin)
	char           *pin_rcv;
char           *calcul_pin;
{
	char            sLine[MAX_LINE_TRC];
...
/*
int 
	Load_Response_Label(V_response_code, V_response_label)
	int             V_response_code;
...
/*
void 
	Load_Balance_Amount(add_amount, bal_amount, currency)
	char           *add_amount;
char           *bal_amount;
char           *currency;
{
...
*/
/*
GetAccountWording(nIndexCtx, pAccType, pAccWording)
	char            pAccType[3];
char            pAccWording[21];
{
	char            sLine[MAX_LINE_TRC];
	...
	for (cpt = 0; cpt < 8; cpt++) {
		if (memcmp(pAccType, tab_NdcCtx[nIndexCtx].gAccountList[cpt].accountType, 2) == 0) {
			memcpy(pAccWording, tab_NdcCtx[nIndexCtx].gAccountList[cpt].accountWording,
				strlen(tab_NdcCtx[nIndexCtx].gAccountList[cpt].accountWording));
			retour = OK;
			break;
		}
	}
...
int Download_FieldGS(char *Buffer,int  Field_Number,char *Field,int  Lg)
{
	char      sLine[MAX_LINE_TRC];
	...
	sprintf(sLine, "Debut Download_FieldGS(%d)", Field_Number);
	trace_event(sLine, PROCESSING);

	memset(tmp_buf,0,sizeof(tmp_buf));

	while ((nb_GS < Field_Number) && (i < Lg-1))
	{
		if (Buffer[i] == GS) nb_GS += 1;
		i += 1;
	}

	if ((nb_GS < Field_Number) || (i == Lg))
	{
		trace_event("Fin   Download_FieldGS(NOK)", PROCESSING);
		return(NOK);
	}
	j = i;
	while ((Buffer[j] != GS)  &&  (j < Lg))
		j+=1;
	...
	if(debug_option == 1 )
		sprintf(sLine, "Fin   Download_FieldGS(%.30s)", Field);
	else
		sprintf(sLine, "Fin   Download_FieldGS(Field len %d)", len);
	trace_event(sLine, PROCESSING);
	return(OK);
}

/**
int Set_Field(Buffer, Field_Number, Field, Lg)
	char *Buffer;
int  Field_Number;
char *Field;
int  Lg;

{
	...
	sprintf(sLine, "Debut Set_Field(%d)", Field_Number);
	...
	if ((nb_FS < Field_Number) || (i == Lg))
	{
		trace_event("Fin   Set_Field(NOK)", PROCESSING);
		return(NOK);
	}
	...
	memcpy(tmp_buf, Buffer, i);
	memcpy(tmp_buf+i, Field, j - i);
	memcpy(tmp_buf+j, Buffer+j, Lg - j);

	memset(Buffer, 0, sizeof(Buffer));
	memcpy(Buffer, tmp_buf, Lg);
	sprintf(sLine, "Fin   Set_Field(%.30s)", Field);
	trace_event(sLine, PROCESSING);
	return(OK);
}
*/
