...
int ReadLineMsg(int   nFd,char *sBuffer, int *nLength)
{
   ...
   *nLength = 0;
   do
   {
       nRead = recv(nFd, sBuffRcv, 8 , 0);
   } while ( (nRead !=8) && (nRead > 0));
   if (nRead < 0)
   {
   ...
   nLenMsg = ((unsigned char)sBuffRcv[6] * 256) + (unsigned char)sBuffRcv[7] ;
   sprintf(sLine,"NLENMSG = %d BUFF = %02X %02X \n", nLenMsg, (unsigned char)sBuffRcv[6], (unsigned char)sBuffRcv[7]);
   ...
   memcpy( sBuffer, sBuffRcv   , nLenMsg-8 );
   *nLength = nLenMsg;
   return (OK);
}
...
int WriteLineMsg(int nFd,char *sBuffer, int nLength)
{
   ...
   sBuffSnd [ 4 ] = 0x00;
   sBuffSnd [ 5 ] = 0x00;
   sBuffSnd [ 6 ] = (nLength+8 ) / 256;
   sBuffSnd [ 7 ] = (nLength+8 ) % 256;
   memcpy (sBuffSnd +8 , sBuffer, nLength);
   retour = write ( nFd, sBuffSnd, nLength+8);
   if (retour <= 0)
   {
      fprintf(stderr,"Error Sending Msg : %d \n", errno);
      return(NOK);
