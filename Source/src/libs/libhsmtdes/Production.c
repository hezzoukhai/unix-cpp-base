...
#include <prod_var.h>
#include <signal.h>
#include <p7_define.h> /*NAB18022019*/
...
/******************************************************************************/
char           *
	GetProdErrorText(char *err_code)
{
	int             i;
...
int 
	ProductionErrorStatus(int nIndiceCtx, char *sTlvBuffer, char *error_code)
{
	char            error_text[30];
...
/**************************************************************************************/
int 
	Cvv1Calculation(int nIndiceCtx, char *sTlvBuffer)
{
	int             nLength;
	char            ligne[MAX_LINE_TRC];
	char            sPan[22 + 1];
...
/**************************************************************************************/
int 
	Cvv2Calculation(int nIndiceCtx, char *sTlvBuffer)
{
	int             nLength;
	char            sPan[22 + 1];
	char            sCVKA[SIZE_KEY + 1], sCVKB[SIZE_KEY + 1];
...
int PvvCalculation(int nIndiceCtx, char *sTlvBuffer)
{
	int             nLength;
	char            sPan[22 + 1];
	char            sPVKA[SIZE_KEY_TDES + 1];/*NAB18022019 Key Block*/
	...
	char            sPVKIndex[1 + 1];
	char            pin_lmk[PIN_LMK_MAX_LEN + 1];
	char            sPVV[4 + 1];

	trace_event("Start  PvvCalculation()", PROCESSING);
	...
	memset(sPVKIndex      , 0 , sizeof(sPVKIndex));
	memset(BuffSnd        , 0 , sizeof(BuffSnd));
	memset(BuffRcv        , 0 , sizeof(BuffRcv));
	/*---------------------------------------*/
	...
	if(sPVKA[0]!='S')/*NAB18022019 Key Block*/
	{
	...
	{
		trace_event("pin_lmk Index is Missing", ERROR);
	...
	memset(sPan, 0, sizeof(sPan));
	memset(sPVV, 0, sizeof(sPVV));
	trace_event("End   PvvCalculation()", PROCESSING);
	return (OK);
}
...
/**************************************************************************************/
int 
	PinCalculation(int nIndiceCtx, char *sTlvBuffer)
{
	int             nLength;
	char            sPan[22 + 1];
	...
	command_EE(BuffSnd, sPVKA, PIN_DATA_OFFSET, sPan, sDecTable);
	...
	PutProdTlvBuffer(PROD_PIN_LMK_TAG, sTlvBuffer, pin_lmk, strlen(pin_lmk)/*PIN_LMK_LENGTH*/);
...
/**************************************************************************************/
int 
	PinPrint(int nIndiceCtx, char *sTlvBuffer)
{
	int             nLength;
	char            sPan[22 + 1];
	char            sPVKA[SIZE_KEY + 1], sPVKB[SIZE_KEY + 1];
...
/**************************************************************************************/
int 
	Production(int nIndiceCtx, char *sTlvBuffer)
{
	int             nLength;
	...
	trace_event("Start Production()", PROCESSING);
	...
	if (PVV_PROD_FLAG == TO_CALCULATE)
		generate_ABA_PVV(nIndiceCtx, sTlvBuffer);
	...
	if (OFFSET_GENR_FLAG == TO_CALCULATE)
		/*ProdPinOffset(nIndiceCtx, sTlvBuffer); */ /*ZKO 17102019  PROD00072366*/
		ProdPinOffsetOfcustomerPIN(nIndiceCtx, sTlvBuffer); 

	trace_event("End   Production(OK)", PROCESSING);
	return (OK);
}
...
/* PVV  Calculation Procedure                                                         */
/**************************************************************************************/
/*Not used, same job as PvvCalculation()
...
************************************************************************/
int 
	ProdPinOffset(int nIndiceCtx, char *sTlvBuffer)
{
	pid_t           nPid;
	char            sPan[22 + 1];
	char            sPVKA[SIZE_KEY_TDES + 1];/*NAB18022019 Key Block*/
	...
	* ------------------------------------------- 
	*/
	command_DE(BuffSnd, sPVKA, pin_lmk, sPan,sDecTable);
...
int 
	ProdPinOffsetOfcustomerPIN(int nIndiceCtx, 
								char    *sTpk_Zpk,
								char    *sPVK,
								char	*sPinBlk,
								char	*sPinBlkFmt,
				...
				)
{
	...
	* ------------------------------------------- 
	*/
	command_BK(BuffSnd, sPVK, sTpk_Zpk, sPinBlk, sPinBlkFmt, sPan,sDecTable);
	...
	*/
	...
	if (command_BL(BuffRcv, sPinOffset, sHsmResultCode) != OK) 
	{
		trace_event("HSM Processing Malfunction", ERROR);
		return (NOK);
	}
	 ...
	 char	*pPvv	)
{
	...
	/* - Construction du tampon ------------------------------------------ */
	command_FW(lHsmBuffIn,pTpk_Zpk, pPvk, pPinBlk, pPinBlkFmt, pCardNbr, pPvkI);
	...
	if (lRet == HSM_MALFUNCTION)
	{
		...
		trace_event("End   generate_ABA_PVV(SYSTEM_MALFUNCTION)",PROCESSING);
		return(HSM_MALFUNCTION);
	}


	/* - Extraction de la reponse --------------------------------------- */
	lRet = command_FX(lHsmBuffOut, pPvv , lErrCode);
