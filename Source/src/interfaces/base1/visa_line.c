   ...
   CodeRet = retrieval_advice_on(MAIN_THREAD_ID);
	...
	while (1) 
	{
		...
		{	
			...
			CodeRet = ReconnectInterface(MAIN_THREAD_ID);
			if (OK != CodeRet) 
			{
		...
		dump_buffer(sLineMsg.sBuffer, sLineMsg.nLength, 'E', 'L', 'I');

		DumpVisa(sLineMsg.sBuffer);
...
/******************************************************************************/
int 
ReconnectInterface(int P_nIndiceCtx)
{
	TSResources     sResource;
