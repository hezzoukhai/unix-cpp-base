...
int  GetVisaParams (int  nIndexCtx, char* pResourceId)
{
	...
	GetTlvBuffer (BASE1_STATION_ID, szParameters, StationId, &nLength );
	GetTlvBuffer (BASE1_ACQUIRER_ID, szParameters, AcquirerId, &nLength );	  
	GetTlvBuffer (BASE1_COUNTRY, szParameters, CountryCode, &nLength );
	GetTlvBuffer (BASE1_ALPHA_CNTR_1, szParameters, AlphaCode3, &nLength );
	GetTlvBuffer (BASE1_ALPHA_CNTR_2, szParameters, AlphaCode2, &nLength );
	GetTlvBuffer (BASE1_BANK_LOCATION, szParameters, BankLocation, &nLength );
	/*Start IBO20200622 PLUTONL-1967*/
	memset(g_sCaptureMsg, '0', 4);
	if (GetTlvBuffer (BASE1_INC_REQ, szParameters, sDataField, &nLength ) == SUCCESS)
		g_sCaptureMsg[0]=sDataField[0];
	if (GetTlvBuffer (BASE1_OUTG_RESP, szParameters, sDataField, &nLength ) == SUCCESS)
		g_sCaptureMsg[1]=sDataField[0];
	if (GetTlvBuffer (BASE1_OUTG_REQ, szParameters, sDataField, &nLength ) == SUCCESS)
		g_sCaptureMsg[2]=sDataField[0];
	if (GetTlvBuffer (BASE1_INC_RESP, szParameters, sDataField, &nLength ) == SUCCESS)
