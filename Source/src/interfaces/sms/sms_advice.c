...
#include <sms_define.h> /* SNO060815 add response 130 */
#include <p7_sid.h>
					...
					TSSmsInfo * OrigSmsInfo)
{
	...
	if( OrigSmsInfo->nMsgType == 120 || OrigSmsInfo->nMsgType == 121 )
		NewSmsInfo.nMsgType = 130;
	else if( OrigSmsInfo->nMsgType == 220 || OrigSmsInfo->nMsgType == 221 )
		NewSmsInfo.nMsgType = 230;
	...
	for (i = SMS_RESPONSE_CODE + 1; i < MAX_SMS_FIELDS; i++)
	{
		if (SmsEchoedField_0220[i])
		{
			...
			if(i == SMS_PRIV_USE_FIELDS)
			{
				if (GetSmsField (SMS_PRIV_USE_FIELDS, OrigSmsInfo, tmpField, &FieldLen)== SUCCESS)
				{     
					if((tmpField[0]&0xC0)==0xC0)
					{
						...
						AddSmsField(SMS_PRIV_USE_FIELDS, &NewSmsInfo, Priv_Use_Field, 7);
					}
					else if((tmpField[0]&0x80)==0x80)
					{
						...
						AddSmsField(SMS_PRIV_USE_FIELDS, &NewSmsInfo, Priv_Use_Field, 5);
					}
				}
			}
			else
	...
	{
		LogVisaMessage(nIndexCtx, &NewSmsInfo, IsoInfo, OUTG_RESP);
	}*/
...
/****************************************************************************/
int 
	FCT_AUT_ADV_FROM_NW(int nIndexCtx, TSSmsInfo * SmsInfo)
{
	...
	case SEVERE_ERROR:
		if (AdvReplyToNw(nIndexCtx, &IsoInfo, SmsInfo) != OK) 
		{
...
/****************************************************************************/
int FCT_REV_ADV_FROM_NW(int nIndexCtx, TSSmsInfo * SmsInfo)
{
	TSSmsInfo      NewSmsInfo;
	TSIsoInfo       IsoInfo;
	int             result;
	...
	case NOK:
		if (RevAdviceRepToNw(nIndexCtx, &IsoInfo, SmsInfo) != OK) {
	...
	for (i = 0; i < MAX_SMS_FIELDS; i++) 
	{
		...
		default:
			if (SmsEchoedField_0400[i] == 1) 
			{
				...
				if(NewSmsInfo.nMsgType == 430 && (i== SMS_TRANS_AMOUNT || i == SMS_TRANS_CRNCY))
					continue;
	...
	FCT_AUT_ADVICE_TO_NW(int nIndexCtx, TSIsoInfo * IsoInfo, char *sTlvBuffer)
{
	...
	result = AdviceReqToNw(nIndexCtx,IsoInfo, &SmsInfo);
...
int AdviceReqToNw(int nIndexCtx, TSIsoInfo * IsoInfo, TSSmsInfo * SmsInfo)
{
	TSCpsInfo       CpsInfo;
	TSTlvPrivate    TlvInfo;
	...
	{
        AddSmsField(SMS_TRANS_AMOUNT, SmsInfo, isoTrxAmnt , 12);
	}
	...
	{
	    AddSmsField(SMS_XMIT_TIME, SmsInfo, sXmiTime, 10);
	}
	...
	if (GetIsoField(ISO_ADTNL_DATA_PRIV, IsoInfo, tmpField, &tmpFieldLen) == SUCCESS)
	{
		...
		if (GetTlvPrivate(EXTERNAL_STAN_TAG, &TlvInfo, sExtStan, &tmpFieldLen) == SUCCESS)
			AddSmsField(SMS_AUDIT_NBR, SmsInfo, sExtStan, 6);
	}
	...
	{
	    AddSmsField(SMS_MERCHANT_TYPE, SmsInfo, MerchantType, 4);
	}
	...
	{
		IsoToSmsPosMode(isoPosMode, NwPosMode);
		AddSmsField(SMS_POS_ENTRY_MODE, SmsInfo, NwPosMode , 4);
	}
	...
	IsoToSmsPosCondition(isoPosMode, MerchantType, 0, tmpField);
	AddSmsField(SMS_POS_CONDITION, SmsInfo, tmpField, SMS_F025_LEN);	
	...
	AddSmsField (SMS_ACQR_ID, SmsInfo, AquirerBin , strlen(AquirerBin));	
	...
	{
        AddSmsField(SMS_AUTHOR_ID, SmsInfo, tmpField, 6); 
	}
	...
	{ 
		AddSmsField (SMS_OUTLET_NBR, SmsInfo, OutletNumber, 15);
	}
	...
	if (GetIsoField(ISO_POS_DATA, IsoInfo, isoPosMode, &tmpFieldLen) == SUCCESS) 
	{
		...
		AddSmsField(SMS_ADTNL_POS_INFO, SmsInfo, tmpField, strlen(tmpField));
	}
...
int
	FCT_ADV_REP_FROM_NW (int nIndexCtx, TSSmsInfo * SmsInfo)
{
	TSBalExchange   BalMessage;
	TSIsoInfo       IsoInfo;
	TSNodeData      Node;
