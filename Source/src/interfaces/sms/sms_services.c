...
void  FCT_REPORT_STATUS (int nIndexCtx, TSIsoInfo *IsoInfo)
{
   ...
   ReplyIsoMsgToBal( nBalAdm , IsoInfo , "" , "000" );
...
void  FCT_LINE_DOWN (int nIndexCtx, TSIsoInfo *IsoInfo)
{
   ...
   /* update resource activity table to indicate that resource was stopped */
...
int  FCT_KEY_REQUEST (int nIndexCtx, TSIsoInfo * IsoInfo, char  KeyType)
{
	...
	AddSmsField(SMS_XMIT_TIME, &msgInfo, sGmtDate + 2, 10);

	AddSmsField(SMS_AUDIT_NBR , &msgInfo,Stan, 6);

    AddSmsField(SMS_FORWD_ID, &msgInfo, AcquirerId, strlen(AcquirerId));

    AddSmsField(SMS_SECURITY_DATA, &msgInfo,"0000000100000000", 16);
	...
	AddSmsField (SMS_PRIV_USE_FIELDS, &msgInfo, sBuffer, Length);
	/* end field 63 */

	if( KeyType == 'I')
		AddSmsField(SMS_NW_MNGMT_INFO,  &msgInfo, SMS_F070_IWK_REQUEST, 3);
	else
        AddSmsField(SMS_NW_MNGMT_INFO,  &msgInfo, SMS_F070_AWK_INCOMING, 3);

   memset(sBuffer, 0, sizeof(sBuffer));
	Length = SmsBuildMsg(sBuffer + SMS_HDR_LEN, &msgInfo);

	SmsBuildHeader(REQUEST, &(msgInfo.sHeader), NULL, Length, StationId);
	memcpy(sBuffer, (char *) &(msgInfo.sHeader), SMS_HDR_LEN);
	Length += SMS_HDR_LEN;
