...
main(int argc, char * argv[])
{
	...
	memset(PlusPmc, '\0', sizeof(PlusPmc));
	...
	memcpy(ResourceId, argv[1], RESOURCE_ID_LEN);


	ThreadMaskSignals();

	memcpy (sInterfaceMode, "I", 1);
	...
	/**** Creation du Thread de traitement des Messages Outgoing ***/

	if (pthread_create(&(tab_ThrInfo[OUTG_THREAD_ID].tid), NULL, 
		thread_OUTG, NULL) < 0)
	{
		fprintf(stderr,"pthread_create Error %d\n", errno);
		exit(0);
